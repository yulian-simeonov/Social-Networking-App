...
@interface ESVideoDetailViewController()
...
- (CGFloat)tableView:(UITableView *)tableView heightForRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    CGFloat height = [ESActivityCell heightForCellWithName:nameString contentString:commentString cellInsetWidth:kESCellInsetWidth];
    if([object objectForKey:kESActivityVideoFileThumbnailKey]) {
        height = 100;
    }
    return height;
    //        }
...
- (void)objectsDidLoad:(NSError *)error {
    self.canLoadMore = [[ESCache sharedCache] commentCountForPhoto:video].integerValue != self.objects.count;
...
{
    return self.objects.count + (self.canLoadMore ? 1 : 0);
}
...
- (NSArray *)tableView:(UITableView *)tableView editActionsForRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    PFObject *object = [self.objects objectAtIndex:dataItemIndex];    if ([[[object objectForKey:@"fromUser"] objectId] isEqualToString:[[PFUser currentUser] objectId]]) {
