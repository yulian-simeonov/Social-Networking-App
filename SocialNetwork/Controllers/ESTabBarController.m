...
//Below is what displays when a video is uploaded.
...
- (void)imagePickerController:(UIImagePickerController *)picker didFinishPickingMediaWithInfo:(NSDictionary *)info {
    ...
    if (!image && url) {
        ...
        [[ALAssetsLibrary new] assetForURL:info[UIImagePickerControllerReferenceURL] resultBlock:^(ALAsset *asset) {
            ...
            [videoObject saveInBackgroundWithBlock:^(BOOL succeeded, NSError *error) {
                ...
                if (succeeded) {
                    ...
                    [[NSNotificationCenter defaultCenter] postNotificationName:@"videoUploadSucceeds" object:videoObject.objectId];
                }
                else if (error) {
        ...
        */
        [[CLImageEditorTheme theme] setBackgroundColor:[UIColor blackColor]];
        [[CLImageEditorTheme theme] setToolbarColor:[[UIColor whiteColor] colorWithAlphaComponent:1.0]];
...
//    if (cameraDeviceAvailable && photoLibraryAvailable) {
        NSArray *menuItems = @[[[RNGridMenuItem alloc] initWithImage:[UIImage imageNamed:@"button_camera"] title:NSLocalizedString(@"Take Photo", nil)],
