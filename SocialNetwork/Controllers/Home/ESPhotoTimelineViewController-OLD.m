...
//The section below displays the users round profile icon and also their username in the hone feed section.
...
//The below section displays the likes and comments on the home feed section.
- (UIView *)tableView:(UITableView *)tableView viewForFooterInSection:(NSInteger)section {
    ...
    //Below is what displays the number of likes and comments on the home timeline
            ...
            if (!outstandingSectionFooterQueryStatus) {
                ...
                [query findObjectsInBackgroundWithBlock:^(NSArray *objects, NSError *error) {
                    @synchronized(self) {
                        ...
                        //Below displays the numeric count for the comment and likes
                        ...
                        // The line below is what displays like count
                        [headerView.likeImage setTitle:[[[ESCache sharedCache] likeCountForPhoto:photo] description] forState:UIControlStateNormal];
                        
                        // The line below is what displays the comment count
                        [headerView.commentImage setTitle:[[[ESCache sharedCache] commentCountForPhoto:photo] description] forState:UIControlStateNormal];
...
//The section below controls the height of the header above the users photo/video in the home feed section.
...
//The section below controls the height of the footer above the users photo/video in the home feed section.
- (CGFloat)tableView:(UITableView *)tableView heightForFooterInSection:(NSInteger)section {
    if (section == self.objects.count) {
        return 0.0f;
    }
    return 84.0f;   //mod:16.0f
    //return 60.0f;   //mod:16.0f
}

//The section below aligns the users images/videos on the timeline in the home feed section.
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    if (indexPath.section == self.objects.count && self.paginationEnabled) {
        // Load More Cell
        //[self loadNextPage]; //Commented this line out for the auto load more on scroll
    }
    else {
        PFObject *object = [self.objects objectAtIndex:indexPath.section];
        ...
        if ([object objectForKey:@"type"] && [object objectForKey:kESVideoFileKey]) {
            ...
            if (cell.movie.playbackState != MPMoviePlaybackStatePlaying) {
                ...
                [cell.movie prepareToPlay];
                    [cell.movie setFullscreen:YES animated:YES];
                [cell.movie play];
...
//The section below displays users photos/videos in the home feed section.
//This is what displays the user photos on the home timeline.
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForNextPageAtIndexPath:(NSIndexPath *)indexPath {
    ...
    [cell.activityControl startAnimating];
    ...
    dispatch_async(downloadQueue, ^{
        ...
        dispatch_async(dispatch_get_main_queue(), ^{
            [cell.activityControl stopAnimating];
        });
...
//The section below displays the users photo/video on the home timeline section.
...
//Added the below lines for the load more by scroll
// scrollview delegate
