...
- (void) didTapVideoThumbnailImage;
//
...
- (id)initWithStyle:(UITableViewCellStyle)style reuseIdentifier:(NSString *)reuseIdentifier
{
    ...
    if (self) {
        ...
        [mainView addSubview:self.videoThumbnailView];
        //
...
- (void)layoutSubviews {
    ...
    //ADDED for video comment
    //[self.videoThumbnailView setFrame:CGRectMake(self.contentLabel.frame.origin.x + self.contentLabel.frame.size.width,self.contentLabel.frame.origin.y,100,100)];
    //[self.videoThumbnailView setFrame:CGRectMake(self.frame.size.width-35-10,self.contentLabel.frame.origin.y,100,100)];
    [self.videoThumbnailView setFrame:CGRectMake(self.frame.size.width-75-10,self.contentLabel.frame.origin.y,75,75)];
    
    if (self.videoThumbnailFile != nil) {
    UIImageView *videoIcon = [[UIImageView alloc] initWithImage:[UIImage imageNamed:@"PlayButt"]];
    //videoIcon.frame = CGRectMake(self.contentLabel.frame.origin.x + self.contentLabel.frame.size.width,self.contentLabel.frame.origin.y,100,100);
    //videoIcon.frame = CGRectMake(250,self.contentLabel.frame.origin.y,100,100);
    videoIcon.frame = CGRectMake(self.frame.size.width-75-10,self.contentLabel.frame.origin.y,75,75);
    [self addSubview:videoIcon];
    }
 
    //Adding action on videothumbnail image view
    UITapGestureRecognizer *singleTap = [[UITapGestureRecognizer alloc] initWithTarget:self action:@selector(didTapVideoThumbnailImage)];
    singleTap.numberOfTapsRequired = 1;
    [self.videoThumbnailView setUserInteractionEnabled:YES];
    [self.videoThumbnailView addGestureRecognizer:singleTap];
    //
...
//ADDED for video comment
- (void) didTapVideoThumbnailImage{
    if(self.delegate && [self.delegate respondsToSelector:@selector(cell:didTapVideoThumbnailImage:)]){
        [self.delegate cell:self didTapVideoThumbnailImage:self.videoThumbnailFile];
    }
...
//ADDED for video comment
- (void)setContentImage:(PFFile *)contentImageFile{
    
    if(contentImageFile == nil){
        self.videoThumbnailView.image = nil;
        return;
    }
    [contentImageFile getDataInBackgroundWithBlock:^(NSData *data, NSError *error) {
        if (!error) {
